...
class Message(CRUDMixin, ProfileMixin):
    ...
    @integer_property
    def stream_chunk_count(self):
        """已完成转写的块数（用于轮询时判断进度）"""
        pass

    @array_property
    def referenced_script_ids(self):
        """本条消息引用的脚本 ID 列表"""
        pass

    @array_property
    def referenced_scripts_snapshot(self):
        """引用脚本发送时刻的快照列表（含截断后的 content）"""
        pass

    def referenced_scripts_meta(self):
        """供前端展示的标签信息"""
        snapshot = self.referenced_scripts_snapshot or []
        meta = []
    ...
    def dump(self):
        return dict(
            ...
            referenced_script_ids=self.referenced_script_ids or [],
            referenced_scripts_meta=self.referenced_scripts_meta(),
            # 时间戳
            created_at=self.created_at.isoformat() if self.created_at else None,
            updated_at=self.updated_at.isoformat() if self.updated_at else None,
        )
...
class ScriptMedia(CRUDMixin, ProfileMixin, TrackableMixin):
    """脚本AI生成素材（封面/素材图）"""
    ...
    script_id: Mapped[int] = mapped_column(
        BigInteger, ForeignKey('scripts.id'), index=True, comment='脚本ID'
    )
    media_type: Mapped[int] = mapped_column(
        Integer,
        default=ScriptMediaType.COVER.value,
        index=True,
        comment='素材类型：10=封面(3:4竖版+4:3横版), 20=素材(4:3)'
    )
    status: Mapped[int] = mapped_column(
        Integer,
        default=ScriptMediaStatus.PENDING.value,
        index=True,
        comment='状态：10=等待, 20=生成中, 30=完成, 40=失败'
    )
    media_id: Mapped[Optional[int]] = mapped_column(
        BigInteger, ForeignKey('media.id'), nullable=True, comment='生成的媒体文件ID'
    )
    ...
    def generated_items(self):
        """素材批次：各段落生成结果 [{segment_index, segment_title, ai_prompt, media_id, status}]"""
        pass
    ...
    def dump(self):
        ...
        return dict(
            id=self.id,
            script_id=self.script_id,
            media_type=ScriptMediaType.init(self.media_type).dump(),
            status=ScriptMediaStatus.init(self.status).dump(),
            ...
            total_segment_count=self.total_segment_count or 0,
