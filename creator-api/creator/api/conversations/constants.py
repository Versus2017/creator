...
class ScriptMediaType(IntEnum):
    """脚本AI生成素材类型"""
    COVER = 10     # 封面：竖版 3:4 + 横版 4:3
    MATERIAL = 20  # 素材 4:3 横版（1536x1024）


ScriptMediaType.COVER.label = '封面'
ScriptMediaType.MATERIAL.label = '素材'

ScriptMediaType.COVER.size = '2480x3312 / 3312x2480'
ScriptMediaType.COVER.ratio = '4K · 3:4 + 4:3'
ScriptMediaType.MATERIAL.size = '3312x2480'
ScriptMediaType.MATERIAL.ratio = '4K · 4:3'


def _dump_script_media_type(self):
    return dict(name=self.name, label=self.label, value=self.value, size=self.size, ratio=self.ratio)


ScriptMediaType.dump = _dump_script_media_type

register_enum(ScriptMediaType)


class ScriptMediaStatus(IntEnum):
    """脚本AI素材生成状态"""
    PENDING = 10
    PROCESSING = 20
    COMPLETED = 30
...
ScriptMediaStatus.PENDING.label = '等待生成'
ScriptMediaStatus.PROCESSING.label = '生成中'
ScriptMediaStatus.COMPLETED.label = '生成完成'
ScriptMediaStatus.FAILED.label = '生成失败'

register_enum(ScriptMediaStatus)
